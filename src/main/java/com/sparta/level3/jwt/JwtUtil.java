...
// JwtUtil 클래스는 JWT 토큰을 생성하고 검증하는 유틸리티 클래스입니다. 주요 기능으로는 토큰 생성, 이메일 추출, 역할 추출, 토큰 만료 확인 등이 있습니다.
...
public class JwtUtil {
    ...
    public String extractRole(String token) {
        // 토큰을 파싱하고 클레임(Claims)을 추출하여 역할(role)을 반환
        Claims claims = Jwts.parserBuilder().setSigningKey(secretKey).build().parseClaimsJws(token).getBody();
        return claims.get("role", String.class); // 역할 반환
    }
